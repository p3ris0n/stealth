...
describe('language code validation', () => {
  ...
  describe('sanitizeLanguageCode', () => {
    ...
    it('should reject injection attempts', () => {
      expect(() => sanitizeLanguageCode("en'; DROP TABLE--")).toThrow(ValidationError);
    });
  });
...
describe('validateProviderConfig', () => {
  ...
  it('should throw ValidationError for invalid timeout', () => {
    ...
    expect(() => validateProviderConfig({ ...validConfig, timeout: 100000 })).toThrow(ValidationError);
  });
  ...
  it('should throw ValidationError for invalid maxRetries', () => {
    expect(() => validateProviderConfig({ ...validConfig, maxRetries: -1 })).toThrow(ValidationError);
    expect(() => validateProviderConfig({ ...validConfig, maxRetries: 10 })).toThrow(ValidationError);
  });
});
...
describe('security guards', () => {
  describe('isRegexSafe', () => {
    ...
    it('should accept moderate repetition', () => {
  ...
  describe('isPrototypeSafe', () => {
    ...
    it('should reject objects with __proto__', () => {
      expect(isPrototypeSafe({ __proto__: { polluted: true } })).toBe(false);
    });
    ...
    it('should detect nested prototype pollution', () => {
      expect(isPrototypeSafe({ nested: { __proto__: { polluted: true } } })).toBe(false);
    });
